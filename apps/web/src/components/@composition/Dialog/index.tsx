...
const DialogVariants = cva({
	base: `fixed z-50 gap-4 bg-gray-900 p-6 shadow-lg transition-all ease-in-out
	...
	variants: {
		position: {
			...
			right:
				'border-l border-gray-600 bg-gray-900 w-full lg:w-1/2 inset-y-0 right-0 h-full data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right',
...
>(
	(
		...
		<DialogPortal>
			<DialogOverlay className={cn('bg-gray-800/50', classNameForeground)}>
...
const DialogHeader = ({
	...
	<div
		className={cn(
			'relative flex h-10 items-center space-x-4 border-b border-gray-500 pb-4 text-center sm:text-left',
			className
		)}
		{...props}
	>
...
const DialogCloseButton = ({ className }: DialogCloseButtonProps) => (
	...
	>
		<XMarkIcon className='h-6 w-6 stroke-gray-300' />
